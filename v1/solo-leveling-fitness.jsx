...
// Vibrancy system: returns a multiplier 0-1 based on rank index
// E=0, D=1, C=2, B=3, A=4, S=5, S+=6, National=7
function getVibrancy(level) {
  const rankIndex = RANKS.reduce((idx, r, i) => level >= r.minLevel ? i : idx, 0);
  return Math.min(rankIndex / 7, 1);
}

// Interpolate between muted and vivid color based on vibrancy
...
// XP curve back to original: accessible early, ramps later
...
function ShadowMonarchArt({ rank, level }) {
  ...
  return (
    <svg viewBox="0 0 200 230" width="170" height="195" style={{ display: "block", margin: "0 auto" }}>
      ...
      {/* Armor details - more visible at higher ranks */}
    ...
    >
      ...
      <div style={{ flex: 1 }}>
        ...
        <div style={{ fontSize: "0.78rem", color: MAKO.textDim, marginTop: 1, fontFamily: "'Rajdhani', sans-serif" }}>
          {quest.desc}
          {!quest.daily && <span style={{ color: MAKO.warning, marginLeft: 8 }}>• {quest.weeklyTarget}x/week</span>}
        </div>
      </div>
      <div style={{
        ...
        whiteSpace: "nowrap",
      }}>{completed ? "✓ CLEAR" : `+${quest.xp} XP`}</div>
    </div>
  );
}
          ...
          <div key={i} style={{
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
              <div style={{
                ...
                {week.isCurrent ? "⬥ THIS WEEK" : `${week.start.toLocaleDateString("en-US", { month: "short", day: "numeric" })} – ${week.end.toLocaleDateString("en-US", { month: "short", day: "numeric" })}`}
              </div>
              <div style={{
...
export default function SoloLevelingFitness() {
  ...
  // Dynamic ambient glow size based on vibrancy
  ...
  return (
    <div style={{
      ...
      {/* Background grid - more vivid at higher ranks */}
      <div style={{
      ...
      <div style={{ maxWidth: 540, margin: "0 auto", padding: "20px 16px 40px" }}>
        ...
        <div style={{
          ...
          <div style={{ display: "flex", justifyContent: "center", gap: 28, marginTop: 16, fontSize: "0.72rem" }}>
            {[
              ...
              { label: "STREAK", value: `${state.streak}${state.streak > 0 ? " 🔥" : ""}`, color: state.streak > 0 ? MAKO.warning : MAKO.textDim },
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
              <div>
                ...
                <div style={{ fontSize: "0.8rem", color: completedDaily === totalDaily ? MAKO.success : MAKO.textDim, marginTop: 2 }}>
                  {completedDaily === totalDaily ? "✦ All quests cleared" : `${completedDaily}/${totalDaily} completed`}
                </div>
              </div>
            ...
            <div style={{
              ...
              {gymThisWeek < 3 && (
                <div style={{ fontSize: "0.68rem", color: MAKO.warning, marginTop: 8, opacity: 0.7 }}>
                  ⚠ Missing target = {GYM_WEEKLY_PENALTY} XP penalty per missed session
                </div>
              )}
            </div>
        ...
        <div style={{ textAlign: "center", marginTop: 24 }}>
          <button onClick={() => { if (confirm("⚠ SYSTEM RESET: All progress will be lost. Continue?")) dispatch({ type: "RESET" }); }}
            style={{
