...
const QUESTS = [
  { id: "stretch", name: "Stretching Protocol", desc: "Swole-in-Seven Stretch (7 min)", xp: 30, icon: "🧘", daily: true },
  { id: "core", name: "Core Enhancement", desc: "Swole-in-Seven Core (7 min)", xp: 35, icon: "💪", daily: true },
  { id: "pushups", name: "Push-Up Trial", desc: "50 Push-Ups", xp: 40, icon: "⬆️", daily: true },
  { id: "squats", name: "Squat Trial", desc: "50 Squats", xp: 40, icon: "🦵", daily: true },
  { id: "dogwalk", name: "Beast Patrol", desc: "Walk the Dogs", xp: 25, icon: "🐕", daily: true },
  { id: "gym", name: "Dungeon Raid", desc: "Gym: 30 min Incline Treadmill", xp: 60, icon: "🏋️", daily: false, weeklyTarget: 3 },
  { id: "nutrition", name: "Ration Control", desc: "Half-box Goodles max", xp: 20, icon: "🍽️", daily: true },
];
...
function fitnessReducer(state, action) {
  switch (action.type) {
    ...
    case "RESET": return { ...fitnessInitialState, loaded: true };
    default: return state;
  }
}

async function saveFitnessState(state) {
  try {
...
const NUTRITION_RANKS = [
  { name: "F", minLevel: 1, color: "#6b7280", glow: "rgba(107,114,128,0.15)", title: "Starving Rookie", vivid: "#9ca3af" },
  { name: "E", minLevel: 4, color: "#5bafa0", glow: "rgba(91,175,160,0.2)", title: "Calorie Counter", vivid: "#6ee8d4" },
  { name: "D", minLevel: 10, color: "#38b4f0", glow: "rgba(56,180,240,0.25)", title: "Macro Tracker", vivid: "#5dc8f5" },
  { name: "C", minLevel: 18, color: "#e8a838", glow: "rgba(232,168,56,0.3)", title: "Clean Eater", vivid: "#f5c84d" },
  { name: "B", minLevel: 28, color: "#5d96a0", glow: "rgba(93,150,160,0.35)", title: "Nutrition Soldier", vivid: "#6dc8dd" },
  { name: "A", minLevel: 40, color: "#7c6dd8", glow: "rgba(124,109,216,0.4)", title: "Diet Master", vivid: "#a78bfa" },
  { name: "S", minLevel: 55, color: "#22c55e", glow: "rgba(34,197,94,0.5)", title: "Fuel Optimizer", vivid: "#4ade80" },
  { name: "S+", minLevel: 75, color: "#c4b5fd", glow: "rgba(196,181,253,0.55)", title: "Nutrition Sovereign", vivid: "#e2d8ff" },
];
...
// Meals with XP values
const MEALS = [
  {
    id: "breakfast", time: "7–9 AM", name: "Morning Fuel", icon: "🌅", xp: 15,
    items: [
      { id: "b1", text: "4 eggs scrambled w/ veggies", detail: "Tomatoes, onion, garlic, red pepper", protein: 28, cals: 340, status: "keep", xp: 8 },
      { id: "b2", text: "¼ cup pumpkin seeds + yogurt", detail: "Greek yogurt if possible", protein: 18, cals: 280, status: "keep", xp: 6 },
  ...
  {
    id: "lunch", time: "12–1 PM", name: "Midday Refuel", icon: "⚡", xp: 20,
    items: [
      { id: "l1", text: "Actual meal — not optional", detail: "This is the gap wrecking your evening", protein: 0, cals: 0, status: "critical", xp: 10 },
  ...
  {
    id: "snack", time: "3–4 PM", name: "Afternoon Rations", icon: "🎯", xp: 12,
    items: [
      { id: "s1", text: "Protein bar", detail: "Kirkland bars are solid — 21g protein", protein: 21, cals: 190, status: "keep", xp: 6 },
  ...
  {
    id: "dinner", time: "6–7:30 PM", name: "Hello Fresh Raid", icon: "🍽️", xp: 18,
    items: [
  ...
  {
    id: "evening", time: "After 8 PM", name: "Late-Night Defense", icon: "🛡️", xp: 15,
    items: [
...
// Get all nutrition items for XP calculations
...
function nutritionReducer(state, action) {
  switch (action.type) {
    ...
    case "RESET": return { ...nutritionInitialState, loaded: true };
    default: return state;
  }
}

async function saveNutritionState(state) {
  try {
    await window.storage.set("solo-leveling-nutrition-v1", JSON.stringify({
      level: state.level, xp: state.xp, totalXp: state.totalXp,
...
async function loadNutritionState() {
  try {
    const r = await window.storage.get("solo-leveling-nutrition-v1");
    if (r && r.value) return JSON.parse(r.value);
  } catch (e) {}
  return null;
}
    ...
    <div style={{
      ...
      <div style={{ textAlign: "center", animation: "shake 0.5s ease-out" }}>
        <div style={{ fontSize: "0.75rem", letterSpacing: "0.5em", color: MAKO.danger, textTransform: "uppercase", marginBottom: 10, fontFamily: "'Rajdhani', sans-serif" }}>
          ⚠ {type === "nutrition" ? "NUTRITION" : "FITNESS"} PENALTY ⚠
        </div>
...
function Heatmap({ log, totalItems, accentColor, vib }) {
  ...
  return (
    <div style={{ display: "flex", gap: cellGap, justifyContent: "center" }}>
      {weeks.map((week, wi) => (
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1 }}>
        ...
        <div style={{ fontSize: "0.68rem", color: MAKO.textDim, marginTop: 1, fontFamily: "'Rajdhani', sans-serif" }}>
          {quest.desc}
          {!quest.daily && <span style={{ color: MAKO.warning, marginLeft: 6 }}>• {quest.weeklyTarget}x/wk</span>}
        </div>
      </div>
      <div style={{ fontFamily: "'Rajdhani', sans-serif", fontWeight: 800, fontSize: "0.82rem", color: completed ? `${MAKO.success}70` : accentColor, whiteSpace: "nowrap" }}>{completed ? "✓" : `+${quest.xp}`}</div>
    </div>
  );
}
  ...
  return (
    <div
      ...
      style={{
        ...
        background: checked ? `${MAKO.success}08` : hover ? `${MAKO.textMuted}08` : "transparent",
        borderRadius: 6, cursor: "pointer", transition: "all 0.2s",
        borderLeft: checked ? `2px solid ${MAKO.success}50` : "2px solid transparent",
      }}
    >
      <div style={{
        width: 16, height: 16, borderRadius: 4, flexShrink: 0, marginTop: 2,
        border: checked ? `2px solid ${MAKO.success}` : `2px solid ${MAKO.textMuted}50`,
        background: checked ? `${MAKO.success}20` : "transparent",
        display: "flex", alignItems: "center", justifyContent: "center",
        fontSize: "0.6rem", color: MAKO.success, transition: "all 0.2s",
      }}>{checked ? "✓" : ""}</div>
      <div style={{ flex: 1 }}>
        <div style={{
          fontSize: "0.78rem", fontWeight: 600, color: checked ? `${MAKO.success}bb` : MAKO.text,
          textDecoration: checked ? "line-through" : "none", textDecorationColor: `${MAKO.success}40`,
          fontFamily: "'Rajdhani', sans-serif", lineHeight: 1.3,
        }}>
          {item.text}
          {status && <span style={{ marginLeft: 5, fontSize: "0.5rem", fontWeight: 800, color: status.color, background: status.bg, padding: "1px 4px", borderRadius: 3, letterSpacing: "0.06em", verticalAlign: "middle" }}>{status.label}</span>}
        </div>
        ...
        {(item.protein > 0 || item.cals > 0) && (
          <div style={{ display: "flex", gap: 6, marginTop: 2, fontSize: "0.55rem", fontFamily: "'Rajdhani', sans-serif" }}>
            {item.protein > 0 && <span style={{ color: MAKO.protein, fontWeight: 700 }}>{item.protein}g</span>}
            {item.cals > 0 && <span style={{ color: MAKO.textMuted }}>{item.cals} cal</span>}
          </div>
        )}
      </div>
      {showXp && item.xp > 0 && (
        <div style={{ fontFamily: "'Rajdhani', sans-serif", fontWeight: 700, fontSize: "0.7rem", color: checked ? `${MAKO.success}60` : MAKO.teal, whiteSpace: "nowrap" }}>
          {checked ? "✓" : `+${item.xp}`}
        </div>
      )}
    </div>
...
function MealSection({ meal, todayLog, onToggle }) {
  ...
  return (
    <div style={{
      ...
      border: allDone ? `1px solid ${MAKO.success}30` : `1px solid ${MAKO.textMuted}18`,
      borderRadius: 10, overflow: "hidden", marginBottom: 8, transition: "border-color 0.3s",
    }}>
      <div style={{ padding: "8px 12px 6px", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: `1px solid ${MAKO.textMuted}12` }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <div style={{ fontSize: "1.1rem" }}>{meal.icon}</div>
          <div>
            <div style={{ fontFamily: "'Rajdhani', sans-serif", fontWeight: 700, fontSize: "0.88rem", color: allDone ? MAKO.success : MAKO.text }}>{meal.name}</div>
            <div style={{ fontSize: "0.55rem", color: MAKO.textDim, fontFamily: "'Rajdhani', sans-serif", letterSpacing: "0.06em" }}>{meal.time}</div>
          </div>
        </div>
        <div style={{ textAlign: "right" }}>
          <div style={{ fontSize: "0.55rem", color: allDone ? MAKO.success : MAKO.textMuted, fontFamily: "'Rajdhani', sans-serif", fontWeight: 700 }}>{completedCount}/{meal.items.length}</div>
          <div style={{ fontSize: "0.55rem", color: allDone ? MAKO.success : MAKO.teal, fontFamily: "'Rajdhani', sans-serif" }}>{earnedXp}/{mealXp} XP</div>
        </div>
      </div>
...
function PerformanceOverview({ fitnessState, nutritionState, fitnessRank, nutritionRank }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: "0.55rem", letterSpacing: "0.4em", color: MAKO.textMuted, textTransform: "uppercase", marginBottom: 10, textAlign: "center" }}>
      ...
      <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 16, marginBottom: 12 }}>
        <div style={{ textAlign: "center" }}>
          <div style={{ fontSize: "0.5rem", color: MAKO.textMuted, letterSpacing: "0.15em", marginBottom: 2 }}>TODAY'S GRADE</div>
          <div style={{
        ...
        <div style={{ textAlign: "center" }}>
          <div style={{ fontSize: "0.5rem", color: MAKO.textMuted, letterSpacing: "0.15em", marginBottom: 2 }}>COMBINED</div>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        {/* Fitness */}
        <div style={{ flex: 1, padding: "10px", background: `${fitnessRank.color}10`, borderRadius: 8, border: `1px solid ${fitnessRank.color}20` }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
            <span style={{ fontSize: "0.9rem" }}>⚔️</span>
            <span style={{ fontSize: "0.65rem", fontWeight: 700, color: fitnessRank.vivid || fitnessRank.color, letterSpacing: "0.1em" }}>FITNESS</span>
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
            <span style={{ fontSize: "0.55rem", color: MAKO.textDim }}>Today</span>
          ...
          <div style={{ height: 3, background: `${MAKO.textMuted}20`, borderRadius: 2, overflow: "hidden", marginBottom: 6 }}>
            <div style={{ height: "100%", width: `${fitnessTodayPct}%`, background: fitnessRank.vivid || fitnessRank.color, borderRadius: 2 }} />
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.5rem", color: MAKO.textDim }}>
        ...
        {/* Nutrition */}
        <div style={{ flex: 1, padding: "10px", background: `${nutritionRank.color}10`, borderRadius: 8, border: `1px solid ${nutritionRank.color}20` }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
            <span style={{ fontSize: "0.9rem" }}>🍽️</span>
            <span style={{ fontSize: "0.65rem", fontWeight: 700, color: nutritionRank.vivid || nutritionRank.color, letterSpacing: "0.1em" }}>NUTRITION</span>
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
            <span style={{ fontSize: "0.55rem", color: MAKO.textDim }}>Today</span>
          ...
          <div style={{ height: 3, background: `${MAKO.textMuted}20`, borderRadius: 2, overflow: "hidden", marginBottom: 6 }}>
            <div style={{ height: "100%", width: `${nutritionTodayPct}%`, background: nutritionRank.vivid || nutritionRank.color, borderRadius: 2 }} />
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.5rem", color: MAKO.textDim }}>
      ...
      {/* Streaks */}
      <div style={{ display: "flex", justifyContent: "center", gap: 20, marginTop: 10, fontSize: "0.55rem" }}>
        <div style={{ textAlign: "center" }}>
          <span style={{ color: MAKO.textMuted }}>Fitness Streak </span>
          <span style={{ color: fitnessState.streak > 0 ? MAKO.warning : MAKO.textMuted, fontWeight: 700 }}>{fitnessState.streak}{fitnessState.streak > 0 ? "🔥" : ""}</span>
        </div>
        <div style={{ textAlign: "center" }}>
          <span style={{ color: MAKO.textMuted }}>Nutrition Streak </span>
          <span style={{ color: nutritionState.streak > 0 ? MAKO.warning : MAKO.textMuted, fontWeight: 700 }}>{nutritionState.streak}{nutritionState.streak > 0 ? "🔥" : ""}</span>
        </div>
      </div>
    </div>
  );
...
export default function SoloLevelingCombined() {
  ...
  useEffect(() => {
    (async () => {
      const saved = await loadFitnessState();
      fitnessDispatch({ type: "LOAD_STATE", payload: saved || {} });
    })();
  }, []);

  // Save fitness state
  useEffect(() => { if (fitnessState.loaded) saveFitnessState(fitnessState); },
    [fitnessState.level, fitnessState.xp, fitnessState.totalXp, JSON.stringify(fitnessState.log), JSON.stringify(fitnessState.weeklyGymLog), fitnessState.penalties, fitnessState.streak]);

  // Load nutrition state
  useEffect(() => {
    (async () => {
      const saved = await loadNutritionState();
      nutritionDispatch({ type: "LOAD_STATE", payload: saved || {} });
    })();
  }, []);

  // Save nutrition state
  useEffect(() => { if (nutritionState.loaded) saveNutritionState(nutritionState); },
    [nutritionState.level, nutritionState.xp, nutritionState.totalXp, JSON.stringify(nutritionState.log), nutritionState.penalties, nutritionState.streak]);
  ...
  const handleFitnessPenalty = () => {
    const missed = dailyQuests.filter((q) => !fitnessTodayLog.includes(q.id)).length;
    if (missed > 0) fitnessDispatch({ type: "APPLY_PENALTY", amount: DAILY_SKIP_PENALTY * missed });
  };

  const handleNutritionPenalty = () => {
    const missed = TOTAL_NUTRITION_ITEMS - totalChecked;
    const penaltyCount = Math.floor(missed / 3); // Penalty for every 3 missed items
    if (penaltyCount > 0) nutritionDispatch({ type: "APPLY_PENALTY", amount: NUTRITION_SKIP_PENALTY * penaltyCount });
  };
  ...
  const currentAccent = mainTab === "fitness" ? fitnessAccent : nutritionAccent;
  ...
  return (
    <div style={{
      ...
      color: MAKO.text, fontFamily: "'Rajdhani', sans-serif", position: "relative", overflow: "hidden",
    }}>
      <style>{`
      ...
      {nutritionState.animatingXp && nutritionState.xpGain !== 0 && <XpBurst amount={nutritionState.xpGain} onDone={clearNutritionAnimations} color={nutritionAccent} />}
      ...
      <div style={{ maxWidth: 440, margin: "0 auto", padding: "14px 12px 30px" }}>
        ...
        <div style={{
          ...
          background: `${MAKO.abyss}cc`, borderRadius: 10, padding: 3,
          border: `1px solid ${MAKO.textMuted}20`,
        }}>
          {[
            { id: "fitness", label: "FITNESS", icon: "⚔️", color: fitnessRank.vivid || fitnessRank.color },
            { id: "nutrition", label: "NUTRITION", icon: "🍽️", color: nutritionRank.vivid || nutritionRank.color },
          ].map((tab) => (
            <button key={tab.id}
              onClick={() => setMainTab(tab.id)}
              style={{
                flex: 1, padding: "10px 0",
                background: mainTab === tab.id ? `linear-gradient(135deg, ${tab.color}18, ${tab.color}08)` : "transparent",
                border: mainTab === tab.id ? `1px solid ${tab.color}35` : "1px solid transparent",
        ...
        {mainTab === "fitness" && (
          <>
            {/* Player Card */}
            <div style={{
              textAlign: "center", marginBottom: 14, padding: "14px 14px 12px",
              background: `linear-gradient(160deg, ${MAKO.bgLight}cc 0%, ${MAKO.bgCard}cc 50%, ${MAKO.bg}cc 100%)`,
              border: `1px solid ${MAKO.textMuted}20`, borderRadius: 12, position: "relative", overflow: "hidden",
            }}>
              <div style={{ position: "absolute", top: -30, left: "50%", transform: "translateX(-50%)", width: 160 + fitnessVib * 60, height: 160 + fitnessVib * 60, borderRadius: "50%", background: `radial-gradient(circle, ${fitnessAccent} 0%, transparent 60%)`, opacity: 0.1 + fitnessVib * 0.1, pointerEvents: "none" }} />

              <ShadowMonarchArt rank={fitnessRank} level={fitnessState.level} type="fitness" />

              <div style={{ fontSize: "0.5rem", letterSpacing: "0.5em", color: MAKO.textMuted, textTransform: "uppercase", marginTop: 4, marginBottom: 3 }}>HUNTER STATUS</div>
              ...
              <div style={{ display: "inline-flex", alignItems: "center", gap: 8, marginBottom: 2 }}>
                ...
                <div style={{ fontSize: "0.62rem", color: MAKO.textDim, textAlign: "left", lineHeight: 1.2 }}>
                  <div>RANK</div>
                  <div style={{ color: MAKO.text }}>{fitnessRank.title}</div>
                </div>
              </div>

              {nextFitnessRank && <div style={{ fontSize: "0.58rem", color: MAKO.textMuted, marginTop: 2 }}>Next: <span style={{ color: nextFitnessRank.vivid || nextFitnessRank.color }}>{nextFitnessRank.name}</span> at Lv.{nextFitnessRank.minLevel}</div>}

              {/* XP Bar */}
              <div style={{ marginTop: 10 }}>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.58rem", color: MAKO.textDim, marginBottom: 3 }}>
                  <span>EXP</span>
                ...
                <div style={{ height: 4, background: `${MAKO.textMuted}18`, borderRadius: 2, overflow: "hidden" }}>
                  <div style={{ height: "100%", width: `${fitnessXpPercent}%`, background: `linear-gradient(90deg, ${vibrateColor(MAKO.teal, fitnessVib)}, ${vibrateColor(MAKO.ocean, fitnessVib)})`, borderRadius: 2, transition: "width 0.6s ease" }} />
                </div>
              </div>

              {/* Stats */}
              <div style={{ display: "flex", justifyContent: "center", gap: 16, marginTop: 10, fontSize: "0.62rem" }}>
                {[
                  { label: "XP", value: fitnessState.totalXp.toLocaleString(), color: MAKO.text },
                  { label: "STREAK", value: `${fitnessState.streak}${fitnessState.streak > 0 ? "🔥" : ""}`, color: fitnessState.streak > 0 ? MAKO.warning : MAKO.textDim },
                  { label: "PENALTY", value: fitnessState.penalties, color: fitnessState.penalties > 0 ? MAKO.danger : MAKO.textDim },
                ].map((s, i) => (
                  <div key={i} style={{ textAlign: "center" }}>
                    <div style={{ color: MAKO.textMuted, letterSpacing: "0.08em", marginBottom: 1, fontSize: "0.5rem" }}>{s.label}</div>
            ...
            <div style={{
              ...
              border: `1px solid ${MAKO.textMuted}15`, borderRadius: 8,
              borderLeft: `3px solid ${fitnessAccent}40`,
            }}>
              <div style={{ fontSize: "0.72rem", color: MAKO.text, fontStyle: "italic", lineHeight: 1.4, fontWeight: 500 }}>"{quote.text}"</div>
              <div style={{ fontSize: "0.5rem", color: MAKO.textMuted, marginTop: 2, letterSpacing: "0.1em", textTransform: "uppercase" }}>— THE SYSTEM {quote.type === "funny" ? "🗿" : "⚔️"}</div>
            </div>
            ...
            <div style={{ display: "flex", gap: 0, marginBottom: 12, background: `${MAKO.bgCard}80`, borderRadius: 8, padding: 2, border: `1px solid ${MAKO.textMuted}15` }}>
              {[{ id: "quests", label: "QUESTS", icon: "⚔️" }, { id: "progress", label: "PROGRESS", icon: "📊" }].map((tab) => (
                <button key={tab.id}
                  onClick={() => fitnessDispatch({ type: "SET_FITNESS_TAB", tab: tab.id })}
                  style={{
                    flex: 1, padding: "7px 0",
                    background: fitnessState.fitnessTab === tab.id ? `linear-gradient(135deg, ${fitnessAccent}12, ${fitnessAccent}06)` : "transparent",
                    border: fitnessState.fitnessTab === tab.id ? `1px solid ${fitnessAccent}28` : "1px solid transparent",
                    borderRadius: 6, cursor: "pointer", fontFamily: "'Rajdhani', sans-serif", fontWeight: 700,
                    fontSize: "0.68rem", letterSpacing: "0.1em",
                    color: fitnessState.fitnessTab === tab.id ? fitnessAccent : MAKO.textMuted, transition: "all 0.2s",
                  }}
                >{tab.icon} {tab.label}</button>
              ))}
            </div>
            ...
            {fitnessState.fitnessTab === "quests" && (
              <>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                  <div>
                    <div style={{ fontSize: "0.55rem", letterSpacing: "0.3em", color: MAKO.textMuted, textTransform: "uppercase" }}>DAILY QUESTS</div>
                    <div style={{ fontSize: "0.7rem", color: completedDaily === totalDaily ? MAKO.success : MAKO.textDim, marginTop: 1 }}>
                      {completedDaily === totalDaily ? "✦ All cleared" : `${completedDaily}/${totalDaily} completed`}
                    </div>
                  </div>
                  <div style={{ fontSize: "0.58rem", color: MAKO.textMuted }}>{new Date().toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric" })}</div>
                </div>

                <div style={{ display: "flex", flexDirection: "column", gap: 5, marginBottom: 12 }}>
                  {QUESTS.map((quest) => <QuestCard key={quest.id} quest={quest} completed={fitnessTodayLog.includes(quest.id)} onToggle={handleFitnessToggle} rank={fitnessRank} vib={fitnessVib} />)}
                </div>

                {/* Gym Progress */}
                <div style={{ padding: "10px 12px", background: `linear-gradient(135deg, ${MAKO.bgCard}cc 0%, ${MAKO.bg}cc 100%)`, border: `1px solid ${MAKO.textMuted}20`, borderRadius: 10, marginBottom: 12 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div>
                      <div style={{ fontSize: "0.55rem", letterSpacing: "0.2em", color: MAKO.textMuted, textTransform: "uppercase" }}>WEEKLY RAIDS</div>
                      <div style={{ fontSize: "1rem", fontWeight: 800, marginTop: 2, color: gymThisWeek >= 3 ? MAKO.success : gymThisWeek > 0 ? MAKO.warning : MAKO.textMuted }}>{gymThisWeek} / 3</div>
                    </div>
                    <div style={{ display: "flex", gap: 4 }}>
                      {[0, 1, 2].map((i) => (
                        <div key={i} style={{
                          width: 20, height: 20, borderRadius: 5,
                          background: i < gymThisWeek ? `linear-gradient(135deg, ${vibrateColor(MAKO.teal, fitnessVib)}, ${vibrateColor(MAKO.ocean, fitnessVib)})` : `${MAKO.textMuted}12`,
                ...
                {completedDaily < totalDaily && (
                  <div style={{ textAlign: "center", marginBottom: 12 }}>
                    <div style={{ fontSize: "0.55rem", color: MAKO.danger, letterSpacing: "0.15em", marginBottom: 5, animation: "pulse 2s infinite" }}>
                      {totalDaily - completedDaily} QUEST{totalDaily - completedDaily > 1 ? "S" : ""} REMAINING
                    </div>
                    <button onClick={handleFitnessPenalty} style={{
                      background: `${MAKO.danger}12`, border: `1px solid ${MAKO.danger}30`,
                      borderRadius: 6, padding: "7px 16px", color: MAKO.danger,
                      fontFamily: "'Rajdhani', sans-serif", fontWeight: 700,
                      fontSize: "0.68rem", letterSpacing: "0.08em", cursor: "pointer", transition: "all 0.2s",
                    }}>END DAY ({DAILY_SKIP_PENALTY * (totalDaily - completedDaily)} XP)</button>
                  </div>
                )}

                {/* Rank Progression */}
                <div style={{ padding: "10px 12px", background: `linear-gradient(135deg, ${MAKO.bgCard}cc 0%, ${MAKO.bg}cc 100%)`, border: `1px solid ${MAKO.textMuted}20`, borderRadius: 10 }}>
                  <div style={{ fontSize: "0.55rem", letterSpacing: "0.2em", color: MAKO.textMuted, textTransform: "uppercase", marginBottom: 6 }}>RANK PROGRESSION</div>
            ...
            {fitnessState.fitnessTab === "progress" && (
              <>
                <div style={{ padding: "12px", marginBottom: 10, background: `linear-gradient(135deg, ${MAKO.bgCard}cc 0%, ${MAKO.bg}cc 100%)`, border: `1px solid ${MAKO.textMuted}20`, borderRadius: 10 }}>
                  <div style={{ fontSize: "0.55rem", letterSpacing: "0.2em", color: MAKO.textMuted, textTransform: "uppercase", marginBottom: 8 }}>TRAINING HEATMAP</div>
                  <Heatmap log={fitnessState.log} totalItems={TOTAL_QUESTS} accentColor={MAKO.teal} vib={fitnessVib} />
                </div>
                ...
                <div style={{ display: "flex", gap: 5, marginBottom: 10 }}>
                  {(() => {
                    ...
                    return [
                      ...
                      <div key={i} style={{ flex: 1, textAlign: "center", padding: "8px 4px", background: `linear-gradient(135deg, ${MAKO.bgCard}aa 0%, ${MAKO.bg}aa 100%)`, border: `1px solid ${MAKO.textMuted}15`, borderRadius: 8 }}>
                        <div style={{ fontSize: "0.48rem", letterSpacing: "0.12em", color: MAKO.textMuted, marginBottom: 2 }}>{stat.label}</div>
        ...
        {mainTab === "nutrition" && (
          <>
            {/* Header */}
            <div style={{
              textAlign: "center", marginBottom: 14, padding: "14px 12px",
              background: `linear-gradient(160deg, ${MAKO.bgLight}cc 0%, ${MAKO.bgCard}cc 100%)`,
              border: `1px solid ${MAKO.textMuted}20`, borderRadius: 12, position: "relative", overflow: "hidden",
            }}>
              <div style={{ position: "absolute", top: -30, left: "50%", transform: "translateX(-50%)", width: 150, height: 150, borderRadius: "50%", background: `radial-gradient(circle, ${nutritionAccent}20 0%, transparent 60%)`, pointerEvents: "none" }} />

              <ShadowMonarchArt rank={nutritionRank} level={nutritionState.level} type="nutrition" />

              <div style={{ fontSize: "0.5rem", letterSpacing: "0.5em", color: MAKO.textMuted, textTransform: "uppercase", marginTop: 4, marginBottom: 3 }}>NUTRITION STATUS</div>
              ...
              {nextNutritionRank && <div style={{ fontSize: "0.58rem", color: MAKO.textMuted, marginTop: 2 }}>Next: <span style={{ color: nextNutritionRank.vivid || nextNutritionRank.color }}>{nextNutritionRank.name}</span> at Lv.{nextNutritionRank.minLevel}</div>}
              ...
              <div style={{ marginTop: 10 }}>
                ...
                <div style={{ height: 4, background: `${MAKO.textMuted}18`, borderRadius: 2, overflow: "hidden" }}>
                  <div style={{ height: "100%", width: `${nutritionXpPercent}%`, background: `linear-gradient(90deg, ${vibrateColor(nutritionAccent, nutritionVib)}, ${vibrateColor(MAKO.hydra, nutritionVib)})`, borderRadius: 2, transition: "width 0.6s ease" }} />
                </div>
              </div>
              ...
              <div style={{ display: "flex", justifyContent: "center", gap: 16, marginTop: 10, fontSize: "0.62rem" }}>
                {[
                  ...
                  { label: "TODAY", value: `${todayNutritionXp}/${MAX_DAILY_NUTRITION_XP}`, color: nutritionAccent },
                  { label: "STREAK", value: `${nutritionState.streak}${nutritionState.streak > 0 ? "🔥" : ""}`, color: nutritionState.streak > 0 ? MAKO.warning : MAKO.textDim },
                ].map((s, i) => (
                  <div key={i} style={{ textAlign: "center" }}>
                    <div style={{ color: MAKO.textMuted, letterSpacing: "0.08em", marginBottom: 1, fontSize: "0.5rem" }}>{s.label}</div>
                    <div style={{ color: s.color, fontWeight: 700, fontSize: "0.82rem" }}>{s.value}</div>
            ...
            <div style={{ display: "flex", gap: 0, marginBottom: 12, background: `${MAKO.bgCard}80`, borderRadius: 8, padding: 2, border: `1px solid ${MAKO.textMuted}15` }}>
              {[
                { id: "meals", label: "MEALS", icon: "🍽️", count: `${checkedMealCount}/${ALL_MEAL_ITEMS.length}` },
                { id: "hydration", label: "WATER", icon: "💧", count: `${checkedHydration}/${HYDRATION.length}` },
                ...
                <button key={t.id} onClick={() => nutritionDispatch({ type: "SET_NUTRITION_TAB", tab: t.id })} style={{
                  flex: 1, padding: "7px 0",
                  background: nutritionState.nutritionTab === t.id ? `linear-gradient(135deg, ${nutritionAccent}12, ${nutritionAccent}06)` : "transparent",
                  border: nutritionState.nutritionTab === t.id ? `1px solid ${nutritionAccent}28` : "1px solid transparent",
                  borderRadius: 6, cursor: "pointer", fontFamily: "'Rajdhani', sans-serif", fontWeight: 700,
                  fontSize: "0.62rem", letterSpacing: "0.06em",
                  color: nutritionState.nutritionTab === t.id ? nutritionAccent : MAKO.textMuted, transition: "all 0.2s",
              ...
              <div>
                ...
                {nutritionTodayPct < 100 && (
                  <div style={{ textAlign: "center", marginTop: 10 }}>
                    ...
                    <button onClick={handleNutritionPenalty} style={{
                      background: `${MAKO.danger}12`, border: `1px solid ${MAKO.danger}30`,
                      borderRadius: 6, padding: "7px 16px", color: MAKO.danger,
                      fontFamily: "'Rajdhani', sans-serif", fontWeight: 700,
                      fontSize: "0.68rem", letterSpacing: "0.08em", cursor: "pointer", transition: "all 0.2s",
                    }}>END DAY ({NUTRITION_SKIP_PENALTY * Math.floor((TOTAL_NUTRITION_ITEMS - totalChecked) / 3)} XP)</button>
                  </div>
                )}
              </div>
            )}
            ...
            {nutritionState.nutritionTab === "hydration" && (
              <div style={{ background: `linear-gradient(135deg, ${MAKO.bgCard}cc 0%, ${MAKO.bg}cc 100%)`, border: `1px solid ${MAKO.textMuted}18`, borderRadius: 10, overflow: "hidden" }}>
                <div style={{ padding: "10px 12px", borderBottom: `1px solid ${MAKO.textMuted}12`, display: "flex", alignItems: "center", gap: 8 }}>
                  <div style={{ fontSize: "1.1rem" }}>💧</div>
                  <div>
                    <div style={{ fontWeight: 700, fontSize: "0.88rem", color: MAKO.hydra }}>Hydration Protocol</div>
                    <div style={{ fontSize: "0.55rem", color: MAKO.textDim, letterSpacing: "0.06em" }}>TARGET: 80 OZ / DAY</div>
                  </div>
                ...
                <div style={{ padding: "3px 2px" }}>
                  {HYDRATION.map(h => (
                    <div key={h.id} onClick={() => handleNutritionToggle(h.id)} style={{
                      ...
                      }}>{nutritionTodayLog.includes(h.id) ? "✓" : ""}</div>
                      <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: "0.58rem", color: MAKO.textMuted, fontWeight: 600 }}>{h.time}</div>
                      <div style={{ fontSize: "0.65rem", color: nutritionTodayLog.includes(h.id) ? `${MAKO.hydra}60` : MAKO.hydra, fontWeight: 700 }}>{nutritionTodayLog.includes(h.id) ? "✓" : `+${h.xp}`}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {nutritionState.nutritionTab === "rules" && (
              <div>
                <div style={{ background: `linear-gradient(135deg, ${MAKO.bgCard}cc 0%, ${MAKO.bg}cc 100%)`, border: `1px solid ${MAKO.warning}20`, borderRadius: 10, overflow: "hidden", marginBottom: 10 }}>
                  <div style={{ padding: "8px 12px", borderBottom: `1px solid ${MAKO.textMuted}12`, display: "flex", alignItems: "center", gap: 8 }}>
                    <div style={{ fontSize: "1rem" }}>⚡</div>
                    <div>
                      <div style={{ fontWeight: 700, fontSize: "0.88rem", color: MAKO.warning }}>Alani Nu Protocol</div>
                      <div style={{ fontSize: "0.55rem", color: MAKO.textDim, letterSpacing: "0.06em" }}>CAFFEINE MANAGEMENT</div>
                    </div>
                  </div>
                  <div style={{ padding: "3px 2px" }}>
                ...
                {/* Rank Progression */}
                <div style={{ padding: "10px 12px", background: `linear-gradient(135deg, ${MAKO.bgCard}cc 0%, ${MAKO.bg}cc 100%)`, border: `1px solid ${MAKO.textMuted}20`, borderRadius: 10 }}>
                  <div style={{ fontSize: "0.55rem", letterSpacing: "0.2em", color: MAKO.textMuted, textTransform: "uppercase", marginBottom: 6 }}>NUTRITION RANKS</div>
                  <div style={{ display: "flex", gap: 3, flexWrap: "wrap" }}>
            ...
            {nutritionState.nutritionTab === "progress" && (
              <>
                <div style={{ padding: "12px", marginBottom: 10, background: `linear-gradient(135deg, ${MAKO.bgCard}cc 0%, ${MAKO.bg}cc 100%)`, border: `1px solid ${MAKO.textMuted}20`, borderRadius: 10 }}>
                  <div style={{ fontSize: "0.55rem", letterSpacing: "0.2em", color: MAKO.textMuted, textTransform: "uppercase", marginBottom: 8 }}>NUTRITION HEATMAP</div>
                  <Heatmap log={nutritionState.log} totalItems={TOTAL_NUTRITION_ITEMS} accentColor={nutritionAccent} vib={nutritionVib} />
                </div>
        ...
        {/* Footer */}
        <div style={{ textAlign: "center", marginTop: 16, fontSize: "0.5rem", color: MAKO.textMuted, letterSpacing: "0.15em" }}>
